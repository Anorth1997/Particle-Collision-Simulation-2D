            ...
            for (int i = -1; i <= 1; i++) { // iterate 3 times
                for (int j = -1; j <= 1; j++) { // iterate 3 times
                    ...
                    if (fixed_x >= 0 && fixed_x < length && fixed_y >= 0 && fixed_y < length) { // valid grid
                        ...
                        grid_to_interact.handle_particle_given_cell(cur_node, particles, dmin,davg,navg);
                    }
                }
            }
            cur_node = cur_node->next;
...
int main( int argc, char **argv )
{
    ...
    for( int step = 0; step < NSTEPS; step++ )
    {
        ...
        // 1. allocate every particle to its belonging grid. O(n)
        // 2. for each grid, check the particles inside it, then compute the nearby grids. O(n)
        // 3. free the nodes
        //
        ...
        // handle the particles grid order
        for (int i = 0; i < length * length; i++) {
            int x = i / length;
            int y = i % length;

            Grid grid = grids[x * length + y];
            // handle all the particles in this grid
            grid.compute_all_particles_in_cell(grids, particles, x, y, length, &dmin,&davg,&navg);
        }
